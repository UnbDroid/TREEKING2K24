...
class WindowCapture:
    ...
    def __init__(self, window_name="", size=(818, 640), origin=(0, 0)):
        ...
        if window_name == "":
            self.hwnd = None  # self.get_screen_position((size[0], size[1]))
            self.w = self.size[0]  # - self.size[0]  # window_rect[2] - window_rect[0]
            self.h = self.size[1]  # - self.size[1]  # window_rect[3] - window_rect[1]
        else:
            # find the handle for the window we want to capture
