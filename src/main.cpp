...
double rps_left = 0;
double rps_right = 0;
int rpm_referencia_left = 80;
int rpm_referencia_right = 80;
double rps_max = 5.75;
...
void loop() {
  // set target position
  //int target = 1200;
  int encoder_volta_left = 1044;
  int encoder_volta_right = 1044; //408
 ...
 // PID constants - RIGHT
  ...
  // error
  double e_left = rpm_referencia_left - (rps_left*60);
  double e_right = rpm_referencia_right - (rps_right*60);
  ...
  Serial.print(rpm_referencia_left);
  Serial.print(" ");
  Serial.print(rpm_referencia_right);
  Serial.print(" ");
